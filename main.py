...
MQTT_BASE_TOPIC = "zigbee2mqtt/+"  # Abonniert automatisch ALLE Sensoren
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # 1. Messwert dauerhaft in SQLite-Datenbank speichern
        save_measurement(
        ...
        # 2. Daten für das Live-Diagramm aktualisieren
        with lock:
...
def erstelle_diagramm():
    ...
    def aktualisieren(frame):
        ...
        if not temps:
            # Wenn noch keine Daten da sind, Platzhalter anzeigen
            ax_temp.cla()
        ...
        ax_temp.cla()
        ax_temp.set_facecolor("#2a2a3e")
        ax_temp.plot(zeiten, temps, color="#f38ba8", linewidth=2.0,
        ...
        ax_temp.set_title(f"Temperatur [{s_name}]", color="#cdd6f4", fontsize=11, fontweight="bold")
        ax_temp.tick_params(colors="#aaaacc", labelsize=8)
        ax_temp.spines[:].set_color("#44445a")
        ax_temp.annotate(f"{temps[-1]:.1f} °C",
                         xy=(zeiten[-1], temps[-1]),
                         xytext=(5, 5), textcoords="offset points",
                         color="#f38ba8", fontsize=10, fontweight="bold")
        ...
        ax_hum.cla()
        ax_hum.set_facecolor("#2a2a3e")
        ax_hum.plot(zeiten, hums, color="#89b4fa", linewidth=2.0,
        ...
        ax_hum.set_title(f"Luftfeuchte [{s_name}]", color="#cdd6f4", fontsize=11, fontweight="bold")
